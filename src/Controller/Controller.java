...
public class Controller {
	...
	public Controller() {
		...
		this.m_CustomerListImpl.add(
				new Customer(24, 112233, true, "유민재", "010-3737-2855", "112233-4556677", "dbalsend", "Godyell713"));
		this.m_CustomerListImpl.getAll().get(m_CustomerListImpl.getSize() - 1).setId(m_CustomerListImpl.getSize() + "");
		this.m_CustomerListImpl
				.add(new Customer(24, 123456, true, "유철민", "010-3737-2855", "123456-1234567", "cjfals", "Godyell99"));
		this.m_CustomerListImpl.getAll().get(m_CustomerListImpl.getSize() - 1).setId(m_CustomerListImpl.getSize() + "");
		this.m_CustomerListImpl
				.add(new Customer(24, 112234, true, "황혜경", "010-3737-2855", "112234-5677889", "gPrud", "hk9126"));
		this.m_CustomerListImpl.getAll().get(m_CustomerListImpl.getSize() - 1).setId(m_CustomerListImpl.getSize() + "");
		this.m_CustomerListImpl
				.add(new Customer(24, 135799, true, "김정호", "010-3737-2855", "135799-2468024", "wjdgh", "2011"));
		this.m_CustomerListImpl.getAll().get(m_CustomerListImpl.getSize() - 1).setId(m_CustomerListImpl.getSize() + "");
		this.m_ApplicationForMembershipListImpl.add(
				new ApplicationForMembership("건물 화재 보험", "010-3737-2855", "24", true, "유민재", "대학생", "112233-4556677"));
		this.m_ApplicationForMembershipListImpl.add(
				new ApplicationForMembership("산악 화재 보험", "010-3737-2855", "24", true, "황혜경", "대학생", "112234-5677889"));
		this.m_ApplicationForMembershipListImpl.add(
				new ApplicationForMembership("일반 화재 보험", "010-3737-2855", "24", true, "유철민", "대학생", "123456-1234567"));
		this.m_ApplicationForMembershipListImpl.add(
				new ApplicationForMembership("산악 화재 보험", "010-3737-2855", "24", true, "유민재", "대학생", "112233-4556677"));
		this.m_ApplicationForMembershipListImpl.add(
				new ApplicationForMembership("일반 화재 보험", "010-3737-2855", "24", true, "유민재", "대학생", "112233-4556677"));
		...
		this.m_SubmitUserListImpl.getAll().get(m_SubmitUserListImpl.getSize() - 1)
				.setId(m_SubmitUserListImpl.getSize() + "");
	...
	public boolean applyForMembership(String insurance, String phoneNum, String age, boolean gender, String name,
		...
		if (!checkInputInformation(insurance, phoneNum, age, SSN, name, jop))
			return false;
		boolean result = m_ApplicationForMembershipListImpl
				.add(new ApplicationForMembership(insurance, phoneNum, age, gender, name, jop, SSN));
		return result;
		// 입력 고객 정보 저장까지 포함
	}
	...
	private boolean checkInputInformation(String insurance, String phoneNum, String age, String citizenNum, String name,
		...
		// 파라미터(성별 -> 주민번호)
		// 이름 변경(Im -> In)
		if (!checkKoreanFormat(name))
			return false;
		if (!checkKoreanFormat(insurance))
			return false;
		if (!checkPhoneNumFormat(phoneNum))
			return false;
		if (!(Integer.parseInt(age) > 0 && Integer.parseInt(age) < 120))
			return false;
		if (!checkKoreanFormat(jop))
			return false;
		if (!checkCitizenNumFormat(citizenNum))
			return false;
		return true;
	}
	...
	public boolean checkPhoneNumFormat(String input) {
		...
		if (input.charAt(3) != '-' || input.charAt(8) != '-')
			return false;
		if (!input.substring(0, 3).equals("010"))
			return false;
	...
	public boolean checkDateStrFormat(String input) {
		...
		if (input.charAt(4) != '-' || input.charAt(7) != '-')
			return false;
		if (!checkNumFormat(input.substring(0, 4)))
			return false;
		if (!checkNumFormat(input.substring(5, 7)))
			return false;
		if (!checkNumFormat(input.substring(8, 10)))
			return false;
		return true;
	}
	...
	public boolean checkCustContracted(String name, String phoneNum) {
		...
		for (Contract contract : m_ContractListImpl.getAll()) {
			...
			if (customer.getName().equals(name) &&
					customer.getPhoneNum().equals(phoneNum))
				return true;
		}
		return false;
	}
