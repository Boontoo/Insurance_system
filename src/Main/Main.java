...
public class Main {
	...
	private void payInsuranceFee(Scanner scanner, SubmitUser submitUser) {
		...
		while (true) {
			...
			while (!isContinue) {
				...
				if (Integer.parseInt(inputPay) >= selectedContract.getPaymentAmount())
					isContinue = true;
				else
	...
	private void startUW(Scanner scanner) {
		...
		while (true) {
			...
			if (!automaticExaminationResult)
				System.out.println(result + "자동심사");
			else if (!diagnosticExaminationResult)
				System.out.println(result + "진단심사");
			else if (!imageExaminationResult)
				System.out.println(result + "이미지심사");
			else if (!specialExaminationResult)
				System.out.println(result + "특인심사");
			else
				System.out.println(result + "일반심사");
			System.out.println(
				...
				switch (Integer.parseInt(selectCase)) {
					...
					case 3:
						while (true) {
							...
							Contract selectCont = controller.getContByCustInsId(selectedIns.getInsuranceID(),
									selectCust.getId());
	...
	private void payInsuranceMoney(Scanner scanner) {
		...
		while (!isContinue) {
			...
			if (controller.checkInAccidentList(name, sSN, accidentId))
				isContinue = true;
			else
				System.out.println("입력 형식이 맞지 않습니다(고객이름, 주민번호, 사건번호)");
		}
		isContinue = false;
	...
	private void reportAccident(Scanner scanner) {
		...
		while (!isContinue) {
			...
			if (controller.checkInCustList(name))
				...
				System.out.println("입력 고객이 고객 리스트에 없습니다");
		}
		isContinue = false;
		String phoneNum = "";
		while (!isContinue) {
			...
			if (controller.checkPhoneNumFormat(phoneNum))
				isContinue = true;
			else
				System.out.println("입력 형식에 맞게 입력해 주세요");
		}
		isContinue = false;
		...
		while (!isContinue) {
			...
			selectedID = scanner.nextLine();
			if (controller.checkIdInContract(selectedID, name))
				isContinue = true;
			else
				System.out.println("입력 아이디가 존재하지 않습니다. 다시 입력하세요");
		}
		isContinue = false;
		controller.saveAccident(selectedID, accidentLocation, accidentType);
		AccidentReception recentRecept = controller.enquireAccidentList()
	...
	private void manageExpirationContract(Scanner scanner) {
		...
		while (!isContinue) {
			...
			if (choice.equals("0"))
				return;
			if (controller.checkInContractList(choice))
				isContinue = true;
			else
				System.out.println("알맞는 번호를 선택해 주세요");
		}
		isContinue = false;
		String inputCase = "";
		while (!isContinue) {
			...
			if (inputCase.length() != 0)
				...
				System.out.println("진행 동작을 입력해 주세요");
		}
		isContinue = false;
	...
	private SubmitUser login(Scanner scanner) {
		...
		while (!isContinue) {
			...
			if (submitUser != null)
				...
				System.out.print("해당 고객 및 직원을 찾지 못했습니다. 계속 진행하겠습니까?(1.예 그이외.아니오) : ");
			if (!scanner.nextLine().equals("1"))
				return null;
			if (submitUser != null)
				isContinue = true;
			else
				System.out.println("아이디 및 비밀번호를 다시 입력해 주세요");
		}
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (submitUser.isUserType())
				...
				main.mainForCustomer(scanner, submitUser);
